...
async function savePythonOnly(code: string) {
  ...
  try {
    ...
    if (store.editorMode === 'visual') {
      const serialized = serializeCurrentProtocol();
...
export function OptimizerEditorPanel() {
  ...
  useEffect(() => {
    // Tier 1: Parse from local pythonContent (works for unsaved protocols)
    ...
    getOptimizerDefaults(protocolType).then((result) => {
      if (cancelled) return;
      if (result) {
        setDefaults(result);
      } else {
        setError('Failed to load optimizer defaults. Make sure the protocol is saved and loaded.');
      }
      setIsLoading(false);
    });
