...
type ExperimentParams = Array<Record<string, Record<string, unknown>>>;
...
/**
 * Parse CSV text into the experiment parameters format expected by the backend.
 *
 * Column headers use dot notation: `task_name.param_name`.
 * Each row is one experiment. Empty cells are omitted (supports optional params).
 *
 * Example CSV:
 ...
 *   ]
 */
export function parseExperimentParametersCsv(csv: string): ExperimentParams {
  ...
  return result.data.map((row) => {
    const experiment: Record<string, Record<string, unknown>> = {};
    ...
    for (const header of headers) {
      ...
      if (!experiment[taskName]) {
        experiment[taskName] = {};
      }
      experiment[taskName][paramName] = value;
    }
...
/**
 * Parse experiment parameters from either JSON or CSV text.
 * Auto-detects the format. Throws descriptive errors on invalid input.
 */
export function parseExperimentParameters(text: string): ExperimentParams {
  ...
  if (format === 'json') {
    try {
      ...
      throw new Error('Invalid JSON in experiment parameters');
    }
  }

  if (format === 'csv') {
    return parseExperimentParametersCsv(text);
  }
