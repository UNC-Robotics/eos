...
import { performAutoLayout } from '@/features/protocol-editor/utils/autolayout';
import { getEdgeColors } from '@/lib/constants/theme';
import type { TaskStatusInfo } from '../api/protocolRunDetails';
import type { ProtocolTaskConfig } from '@/lib/api/specs';

interface ProtocolRunFlowCanvasProps {
  protocolTasks: ProtocolTaskConfig[];
...
export function ProtocolRunFlowCanvas({
  protocolTasks,
  taskStatuses,
  onTaskSelect,
  selectedTaskName,
  onShowTaskList,
}: ProtocolRunFlowCanvasProps) {
  ...
  const initialNodes = useMemo((): Node[] => {
    return protocolTasks.map((expTask, index) => {
  ...
  }, [protocolTasks, taskStatusMap]);

  const initialEdges = useMemo((): Edge[] => {
    const edges: Edge[] = [];
    protocolTasks.forEach((expTask) => {
      ...
      <div className="w-full h-full flex items-center justify-center bg-gray-50 dark:bg-slate-800">
        <div className="text-center">
          <p className="text-base text-gray-500 dark:text-gray-400">
            Protocol definition is required to show protocol run DAG.
          </p>
          {onShowTaskList && (
            <button
