...
import { buildFloatParamsMap, ensureFloatNotationInYaml } from '@/lib/utils/experimentHelpers';
import type { TaskNode, TaskSpec } from '@/lib/types/experiment';
...
/**
 * Synchronously serialize the current experiment store state into YAML and layout JSON.
 * Called from both the visual editor sync effect and the save handler to ensure consistency.
 */
export function serializeCurrentExperiment(): SerializedExperiment {
  const { tasks, taskTemplates, experimentType, experimentDesc, labs } = useEditorStore.getState();

  const specMap = new Map(taskTemplates.map((t) => [t.type, t]));
  const tasksForYaml = tasks.map((task) => serializeTaskForYaml(task, specMap));
  let yaml = serializeYaml({ type: experimentType, desc: experimentDesc, labs, tasks: tasksForYaml });
