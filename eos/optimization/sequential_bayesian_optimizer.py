...
class BayesianSequentialOptimizer(AbstractSequentialOptimizer):
    ...
    def _generate_initial_samples_df(self) -> None:
        self._initial_samples_df = self._sample(self._num_initial_samples)

    def _sample(self, n: int) -> pd.DataFrame:
        """Generate samples, respecting domain constraints if any exist."""
        if not self._domain.constraints.constraints:
            return self._domain.inputs.sample(n=n, method=self._initial_sampling_method)

        strategy = RandomStrategy.make(
            domain=self._domain,
            fallback_sampling_method=self._initial_sampling_method,
        )
        return strategy.ask(n)[self._input_names]
