...
class ExperimentManager:
    ...
    async def update_experiment_parameters(
        self, db: AsyncDbSession, experiment_name: str, parameters: dict[str, dict[str, Any]]
    ) -> None:
        """Update the parameters of an existing experiment."""
        await self._validate_experiment_exists(db, experiment_name)
        await db.execute(
            update(ExperimentModel).where(ExperimentModel.name == experiment_name).values(parameters=parameters)
        )

    async def delete_experiment(self, db: AsyncDbSession, experiment_name: str) -> None:
        """Delete an experiment and all associated records."""
        await self._validate_experiment_exists(db, experiment_name)

        # Delete all records that reference this experiment
        await db.execute(delete(TaskModel).where(TaskModel.experiment_name == experiment_name))
        await db.execute(delete(DeviceAllocationModel).where(DeviceAllocationModel.experiment_name == experiment_name))
        await db.execute(
            delete(ResourceAllocationModel).where(ResourceAllocationModel.experiment_name == experiment_name)
        )
        await db.execute(delete(CampaignSampleModel).where(CampaignSampleModel.experiment_name == experiment_name))
    ...
    async def fail_experiment(self, db: AsyncDbSession, experiment_name: str, error_message: str | None = None) -> None:
        """Fail an experiment."""
        await self._set_experiment_status(db, experiment_name, ExperimentStatus.FAILED, error_message=error_message)

    async def get_experiment_priorities(self, db: AsyncDbSession, names: list[str]) -> dict[str, int]:
        if not names:
            return {}
        result = await db.execute(
            select(ExperimentModel.name, ExperimentModel.priority).where(ExperimentModel.name.in_(names))
        )
        return dict(result.all())

    async def fail_experiments_batch(
        ...
        if error_message is not None:
            update_fields["error_message"] = error_message
        await db.execute(update(ExperimentModel).where(ExperimentModel.name.in_(names)).values(**update_fields))

    async def cancel_experiments_batch(self, db: AsyncDbSession, names: list[str]) -> None:
        if not names:
            return
        await db.execute(
            update(ExperimentModel)
            .where(ExperimentModel.name.in_(names))
            .values(status=ExperimentStatus.CANCELLED, end_time=datetime.now(UTC))
        )
